...
@implementation CityAffiliation
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if (tableView == self.table) {
		...
		switch (indexPath.section) {
			case 0://name
				[tf setText:[NSString stringWithFormat:@"%@",editable.cityName]];
				if ([editable.cityName isEqualToString:@"City"])
					[tf setText:[NSString stringWithFormat:@""]];
				[tf setPlaceholder:@"City"];
				[tf setTag:0];
				tf.autocorrectionType = UITextAutocorrectionTypeNo;
				cityField = tf;
				break;
			case 1:
				[tf setPlaceholder:@"Affiliation"];
				[tf setText:[NSString stringWithFormat:@"%@",editable.affiliation]];
				if ([editable.affiliation isEqualToString:@"Affiliation"])
					[tf setText:[NSString stringWithFormat:@""]];
				[tf setTag:1];
				break;
			default:
...
- (void)textFieldDidChangeEditing:(UITextField *)textField{
	switch (textField.tag) {
		case 0:
			editable.cityName = [NSString stringWithFormat:@"%@",textField.text];
			...
			break;
		case 1:
			editable.affiliation = [NSString stringWithFormat:@"%@",textField.text];
			break;
		default:
			break;
	}
